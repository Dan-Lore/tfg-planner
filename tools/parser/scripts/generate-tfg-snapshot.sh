...
JVM_SCRIPT="$SCRIPT_DIR/server-jvm-args.mjs"
MIN_RECIPES=6000
...
cp "$EXPORT_SCRIPT" "$WORK_DIR/kubejs/server_scripts/zzz_tfg_planner_export.js"
node "$SCRIPT_DIR/prepare-server-overrides.mjs" "$WORK_DIR"

PAKKU_JVM_FLAGS=($(node "$JVM_SCRIPT" --pakku-flags))
node "$JVM_SCRIPT"
(cd "$WORK_DIR" && "$JAVA" "${PAKKU_JVM_FLAGS[@]}" -jar pakku.jar -y fetch && "$JAVA" "${PAKKU_JVM_FLAGS[@]}" -jar pakku.jar -y export)
...
cp "$EXPORT_SCRIPT" "$SERVER_RUN/kubejs/server_scripts/zzz_tfg_planner_export.js"
echo "eula=true" > "$SERVER_RUN/eula.txt"

SERVER_JVM_FLAGS=($(node "$JVM_SCRIPT" --server-flags))
SERVER_TIMEOUT_MIN=$(node -p "JSON.parse(require('fs').readFileSync(0,'utf8')).timeoutMin" <<< "$(node "$JVM_SCRIPT" --json)")

EXPORT_FILE="$SERVER_RUN/logs/tfg-planner-recipe-snapshot/recipes.json"
timeout "${SERVER_TIMEOUT_MIN}m" bash -c "cd '$SERVER_RUN' && '$JAVA' ${SERVER_JVM_FLAGS[*]} -jar minecraft_server.jar nogui" || true
