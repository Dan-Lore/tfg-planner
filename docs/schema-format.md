...
Визуальная группировка узлов (рамка, подпись). Поле **сохраняется** в `.tfgp` и проходит валидацию; **редактор групп в UI не реализован** (ручное редактирование JSON или будущая фича). Export из приложения — `groups: []`.
...
| `ratePerSecond` | number | Зафиксированная скорость, items/s или mB/s |

Отсутствие поля = нет закреплений (backward-compatible import).
