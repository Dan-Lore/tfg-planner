...
`capacity` у промежуточного буфера сохраняется при создании (`round(flow × 3600)`) и отображается в UI. В steady-state pass-through **не ограничивает** скорость потоков; при **запуске катализаторной петли** (K-023) используется для bootstrap. Если `capacity` и `initialStock` равны 0, солвер заводит петлю по **теоретическому спросу** рецепта; рекомендуемая ёмкость на 1 ч показывается на seed-ребре в инспекторе.
