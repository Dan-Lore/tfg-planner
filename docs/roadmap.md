...
| Фича | Kanban | Статус |
|------|--------|--------|
| Парсер Modpack-Modern | K-001, K-010 | done |
| Меню версий + pack data | K-001 | done |
| Редактор мнемосхем | K-006, K-014 | done |
| Двусторонний расчёт продуктов | K-002 | done |
| Масштабирование цепочки / группы | K-006 | done |
| Import / export `.tfgp` | — | done |
| i18n RU + EN | K-004 | done |
| Энергия EU/t на узле | K-003 | done |
| Автообновление pack data | K-009 | blocked (ждёт спецуказания) |

**Вне MVP:** облако, аккаунты, публичная витрина, рейтинги.

### Деплой и обновление данных (GitHub Pages)

| Подход | Где | Ресурсы | Обновляет рецепты |
|--------|-----|---------|-------------------|
| Commit `pack.json` вручную | dev → git → Pages | Минимум | Да, при push |
| Проверка `manifest.json` в UI | только браузер | ~1 KB на проверку | Подхватывает уже выложенный pack |
| Scheduled GitHub Actions | CI | ~5–30 мин / прогон | Да (build-pack → commit/PR) |
| Парсер в браузере | нецелевое | Очень много | Теоретически, непрактично |

**Целевая модель:** статический `pack.json` в репозитории; опционально cron-workflow и лёгкая проверка версии в UI. Детали — [kanban.md](kanban.md) **K-009**.
...
| 2026-06-18 | Автообновление pack data — CI + UI-проверка manifest; не парсер в браузере → K-009 |
| 2026-06-18 | Полнота парсера + GT substrate dump → K-010 |
